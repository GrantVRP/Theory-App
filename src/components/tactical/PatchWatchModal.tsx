          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[9px] px-1.5 py-0.2 rounded bg-red-950 text-red-300 border border-red-700 font-pixel-heading">
          ...
          {isLoading ? (
            ...
            filteredChanges.map((change) => {
              ...
              return (
                <div
                  key={change.sha}
                  className="p-3 rounded bg-[#0d101a] border border-zinc-800 hover:border-zinc-700 transition-all space-y-1.5"
                >
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1 text-[8.5px] text-zinc-500 font-pixel-heading">
                  ...
                  <div className="text-xs text-zinc-200 font-pixel-body whitespace-pre-line leading-relaxed pl-5 border-l-2 border-zinc-800">
                    {change.message}
                  </div>
                  ...
                  {change.unitDefs && change.unitDefs.length > 0 && (
                    <div className="flex items-center gap-1 flex-wrap pt-1 pl-5">
                      <span className="text-[8px] font-pixel-heading text-zinc-500 uppercase">
                        Units:
                      </span>
                      {change.unitDefs.map((u, idx) => (
                        <span
                          key={idx}
                          className="px-1.5 py-0.2 rounded bg-amber-950/40 text-amber-300 border border-amber-800/60 text-[8px] font-pixel-heading"
                        >
                          {u.name}
                        </span>
                      ))}
